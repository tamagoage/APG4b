...
void saiten(vector<vector<int>> &A, int &correct_count, int &wrong_count) {
    ...
    vector<vector<int>> correct_answer(9, vector<int>(9));
    // ここにプログラムを追記
    for (int i = 0; i < 9; i++){
        for (int j = 0; j < 9; j++){
            correct_answer.at(i).at(j) = (i + 1) * (j + 1);
            if (A.at(i).at(j) == correct_answer.at(i).at(j)){
                correct_count++;
            } else {
                wrong_count++;
                A.at(i).at(j) = correct_answer.at(i).at(j);
            }
        }   
    }
